...
mask_ip() {
    # 保留前两段，后两段替换为 xx.xx（如 64.83.15.74 → 64.83.xx.xx）
    # 兼容 CIDR 格式（如 64.83.0.0/20 → 64.83.xx.xx/20）
    local raw="$1" cidr="" ip
    case "$raw" in */*) cidr="/${raw#*/}"; raw="${raw%%/*}" ;; esac
    ip=$(echo "$raw" | awk -F. '{printf "%s.%s.xx.xx", $1, $2}')
    echo "${ip}${cidr}"
}

mask_ip6() {
    local raw="$1"
    echo "$raw" | awk -F: '{printf "%s:%s:x:x:x:x:x:x", $1, $2}'
}
...
rkv "公网 IPv4:" "$([ -n "${PUB4:-}" ] && mask_ip "$PUB4" || echo "N/A")"
rkv "公网 IPv6:" "$([ -n "${PUB6:-}" ] && mask_ip6 "$PUB6" || echo "未配置")"
...
if [ -n "$PUB4" ]; then
    ...
    rkv "Prefix:" "$(mask_ip "${PREFIX_CIDR}")  [${COUNTRY}]"
